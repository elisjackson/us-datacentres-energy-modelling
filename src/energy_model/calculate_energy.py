...
def solve(
    ...
    df["Wind Speed"] = df["Wind Speed"].apply(lambda x: custom_round(x, base=0.25))
    # merge power curve
    df = pd.merge(
        df, power_curve, left_on="Wind Speed", right_on="wind_speed", how="left"
    ).fillna(0)
    ...
    pv_ac.name = "PV_AC"
    df = pd.merge(df, pv_ac, how="left", left_index=True, right_index=True).fillna(0)
    ...
    df["demand_after_gen_plus_pv"] = (df["demand"] - df["gen_plus_pv"]).clip(lower=0)
    ...
    gas_fuel_cost = {
        "Gas CCGT": df_GNESTE.loc["Gas CCGT", "fuel_price"]
        * df["gas_consumption"].sum()
    }

    # Combine into one DataFrame
    df_costs_emissions = pd.DataFrame(
        {"CAPEX": capex, "OPEX": opex, "Fuel cost": gas_fuel_cost}
    ).fillna(0)
    ...
    df_costs_emissions["20yr fuel cost"] = (
        df_costs_emissions["Fuel cost"] * lifetime_assumed
    )
    ...
    df_costs_emissions["Lifetime kgCO2"] = (
        lifetime_assumed * df_costs_emissions["kgCO2"]
    )
...
def calculate_battery_profile(
    df: pd.DataFrame, bess_mw: float, bess_mwh: float
) -> pd.DataFrame:
    t0 = time.time()
    ...
    for t in range(1, len(soc)):
        ...
        bess_power[t] = np.clip(
            bess_power[t], -soc_discharge_headroom, soc_charge_headroom
        )
...
def wt_power_curve(wind_farm_mw):
    ...
    power = {10: 5.6, 11: 7.1, 12: 7.8, 13: 8.0, 20: 8.0}

    power_curve_df = pd.DataFrame(
        {"wind_speed": list(power.keys()), "power": list(power.values())}
    )
    ...
    interp_func = interp1d(
        power_curve_df.wind_speed, power_curve_df.power, kind="linear"
    )
    fine_powers = interp_func(fine_wind_speeds)

    # wind_power_single = single wind turbine power output (MW)
    power_curve_df = pd.DataFrame(
        {"wind_speed": fine_wind_speeds, "wind_power_single": fine_powers}
    )

    power_curve_df["farm_wind_power"] = (
        power_curve_df["wind_power_single"]
        * wind_farm_mw
        / power_curve_df["wind_power_single"].max()
    )
...
def get_GNESTE_assumptions(energy_tech: str) -> pd.DataFrame:
    ...
    # capex (USD/kW)
    capex = df[(df["Variable"] == "Capital Cost") & (df["Unit"] == "USD/kW")]
    capex = capex[years]
    ...
    opex = df[
        (
            (df["Variable"] == "Fixed O&M")
            if energy_tech != "Solar PV"
            else (df["Variable"] == "Total O&M")
        )
        & (df["Unit"] == "USD/kW/yr")
    ]
    opex = opex[years]
