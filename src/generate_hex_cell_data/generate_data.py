...
def main():
    ...
    hex_wind_pv_gdf = hex_wind_gdf.merge(
        hex_gti_gdf.drop(columns=["geometry"]), on="hex", how="outer"
    )
    ...
    state_hex_lookup = hex_states_df.groupby("state")["hex"].apply(list).to_dict()
    # save JSON lookup
